...
import { AlertCircle, ClipboardList, Calendar as CalendarIcon, FileText, Wrench } from "lucide-react";
import KpiCard from "../../components/KpiCard.jsx";
import MiniCalendar from "../../components/MiniCalendar.jsx";
import axios from "axios";

export default function InicioCalidad() {
  ...
  return (
    <div className="p-6 text-[#111A3A] grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-6">
      <div>
        ...
        {/* Accesos directos (solo los permitidos para Calidad) */}
        <section>
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold mb-4">Accesos directos</h2>
            <div className="flex flex-wrap gap-4">
              <Link to="/calidad/calendario" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md flex items-center gap-2">
                <CalendarIcon size={16} /> Calendario
              </Link>
              <Link to="/calidad/lista-equipos" className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md flex items-center gap-2">
                <Wrench size={16} /> Lista de equipos
              </Link>
              <Link to="/calidad/auditoria" className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md flex items-center gap-2">
